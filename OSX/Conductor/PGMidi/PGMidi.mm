...
- (void) sendPacketList:(const MIDIPacketList *)packetList
{
    [self sendPacketList:packetList toChannel:0];
}

- (void) sendPacketList:(const MIDIPacketList *)packetList toChannel:(NSUInteger)channel
{
    MIDIEndpointRef outputEndpoint = MIDIGetDestination(channel);
    if (outputEndpoint)
    {
        // Send it
...
{
    [self sendBytes:data size:size channel:0];
}

- (void) sendBytes:(const UInt8*)data size:(UInt32)size channel:(NSUInteger)channel
{
    NSLog(@"%s(%u bytes to core MIDI)", __func__, unsigned(size));
    assert(size < 65536);
    Byte packetBuffer[size+100];
    ...
    [self sendPacketList:packetList toChannel:channel];
}
